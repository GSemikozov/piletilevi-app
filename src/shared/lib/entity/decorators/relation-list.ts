...
const RelationList =
  (getTypeList: () => EntityList<BaseEntity>) => (target: ObjectConstructor, key: string) => {
    const relationsListProperties =
      Reflect.getMetadata(relationsListPropertiesSymbol, target) || [];
